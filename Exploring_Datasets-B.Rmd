...
if(file.exists("Exploring_Datasets-B.html")){
  file.remove("Exploring_Datasets-B.html")
}
...
Note that we simply put the name `titanic` inside the parentheses of the function, like this:

```{r}
htmlOutput("code_1_3")
```

Check out the result of the following lines of code:
...
Perhaps we only want to consider a few of the variables. This can be done using brackets `[]`.  Suppose we want to look at the name of the passenger, and only one other piece of information.  We will select these two variables, and then put the results into `head()` so we can look at the first few values.
...
More often, we want to narrow down our sample according to one of the variables.  A nice shortcut for this is the function `subset()`
...
output$code_filter <- renderText(
        makePrintText(
          base_string = "head(subset(%oi, %ni == %ni))",
          old_input = c("titanic"),
...
To create this, we will use the `$` and `<-` operators to create a new variable in our dataset.
...
output$code_fam <- renderText(
        makePrintText(
          base_string = "titanic$%oi <- titanic$%ni + titanic$%ni",
          old_input = c("Family.Aboard"),
...
output$code_3_1 <- renderText(
        makePrintText(
          base_string = "titanic$%oi <- factor(titanic$%ni)",
          old_input = c("Passenger.Class"),
...
output$code_3_2 <- renderText(  
  makePrintText(
          base_string = "titanic$%oi <- factor(titanic$%oi, \n \t \t levels = c(1,2,3), \n \t \t labels = c('%ni', '%ni', '%ni')) \n )",
